...
function JoinButton({ lobby, session, setJoinButtonDom }: JoinProps) {
  ...
  return (
    <button
      className={`bg-neutral-700 hover:bg-neutral-800 text-white font-bold py-2 px-16 rounded ${(!session || (isLobbyFull && !isPlayerInLobby)) && "cursor-not-allowed"
        }`}
...
export default function PlayersList({
  ...
  return (
    <div>
      <div
        className={`text-3xl font-bold pb-3 ${lobby.players.length === 0 && "py-3"
          }`}
      >
        <span>
          Players:{" "}
            ...
            return (
              <li key={`${player}_${index}`} className="py-3">
                <div className="flex items-center space-x-4">
                  ...
                  <span>
                    <span
                      className={`text-xl font-bold flex-1 min-w-0 ${getDataByUsername(player.username, lobby).userColor
                        } ${lobby.status === "playing" &&
