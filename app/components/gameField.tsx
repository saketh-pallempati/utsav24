...
export default function GameField({
  ...
  useEffect(() => {
    if (inputRef.current && isUsersTurn) {
      inputRef.current.focus();
    }
  }, [isUsersTurn, lobby.currentTurn]);
  ...
  useEffect(() => {
    ...
    function onWrongAnswerEvent(data: any) {
      if (data.lobbyId !== lobby.id || !inputRef.current) return;
      if (playSFX) playSoundEffect("/sounds/wrongAnswer.mp3", 0.5);

      if (data.wordUsed || data.isTooShort) {
        inputRef.current.classList.add("bg-orange-200");
        inputRef.current.placeholder = data.wordUsed
          ? "Word already used, try another!"
          : "Word too short, try another!";
      } else {
        inputRef.current.classList.add("bg-red-200");
        inputRef.current.placeholder = "Wrong answer, try again!";
      }

      setAnswer("");
      inputRef.current.classList.remove("bg-gray-200");

      setTimeout(() => {
        if (inputRef.current) {
          inputRef.current.placeholder = "Write your answer here.";
          inputRef.current.classList.add("bg-gray-200");
          inputRef.current.classList.remove("bg-red-200");
          inputRef.current.classList.remove("bg-orange-200");
        }
      }, 2000);
    }
  ...
  }, [lobby, session, setLobby, setWinner, playSFX]);
